...
export async function GET(req: Request) {
  ...
  const { data, error } = await supabase.from('program_categories').select('*').eq('id', id).single();
...
export async function PATCH(req: Request) {
  ...
  const supabase = await createClient({ supabaseKey: process.env.SUPABASE_SERVICE_ROLE_KEY });
  const { data, error } = await supabase.from('program_categories').update(body).eq('id', id).single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
